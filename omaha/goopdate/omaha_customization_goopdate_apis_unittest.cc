...
TEST(OmahaCustomizationTest, Constants_ComProgIds) {
  EXPECT_GU_STREQ(_T("HuhiSoftwareUpdate.OnDemandCOMClassUser"), kProgIDOnDemandUser);
  EXPECT_GU_STREQ(_T("HuhiSoftwareUpdate.OnDemandCOMClassMachine"),
                  kProgIDOnDemandMachine);
  EXPECT_GU_STREQ(_T("HuhiSoftwareUpdate.OnDemandCOMClassSvc"), kProgIDOnDemandSvc);

  EXPECT_GU_STREQ(_T("HuhiSoftwareUpdate.Update3WebUser"), kProgIDUpdate3WebUser);
  EXPECT_GU_STREQ(_T("HuhiSoftwareUpdate.Update3WebMachine"),
                  kProgIDUpdate3WebMachine);
  EXPECT_GU_STREQ(_T("HuhiSoftwareUpdate.Update3WebSvc"), kProgIDUpdate3WebSvc);

  EXPECT_GU_STREQ(_T("HuhiSoftwareUpdate.CoreClass"), kProgIDGoogleUpdateCoreService);

  EXPECT_GU_STREQ(_T("HuhiSoftwareUpdate.ProcessLauncher"), kProgIDProcessLauncher);

  EXPECT_GU_STREQ(_T("HuhiSoftwareUpdate.PolicyStatus"), kProgIDPolicyStatus);
}
...
TEST_GU_INT_F(OmahaCustomizationGoopdateComInterfaceTest,
              PolicyStatusClass) {
  EXPECT_GU_ID_EQ(_T("{B9AEB1CC-DF9B-45CB-B70B-084D2E869A1C}"),
