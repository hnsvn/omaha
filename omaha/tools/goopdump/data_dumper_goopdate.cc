...
void DataDumperGoopdate::DumpGoogleUpdateIniFile(const DumpLog& dump_log) {
  DumpHeader header(dump_log, _T("HuhiUpdate.ini File Contents"));
  DumpFileContents(dump_log, _T("c:\\huhiupdate.ini"), 0);
}
...
void DataDumperGoopdate::DumpUpdateDevKeys(const DumpLog& dump_log) {
  ...
  DumpRegistryKeyData(dump_log, _T("HKLM\\Software\\HuhiSoftware\\UpdateDev"));
}
...
void DataDumperGoopdate::DumpGoogleUpdateProcessInfo(const DumpLog& dump_log) {
  DumpHeader header(dump_log, _T("HuhiUpdate.exe Process Info"));
  ...
  do {
    ...
    if (exe_file_name.Find(_T("huhiupdate.exe")) >= 0) {
      if (first) {
        first = false;
      } else {
        dump_log.WriteLine(_T("-------------------"));
